...
class TestRequest:
    def __init__(self, server, product, version, folder):
        self.server = server
    ...
    def request(self):
        query = 'domino/active/python/distro.get'
        start = datetime.datetime.now()
        params = {'product_id': self.product}
        self.r = requests.get(f'https://{self.server}/{query}',  params=params)
...
if __name__ == "__main__":
    ...
    r = TestRequest(server, product, None, folder)
    test(r)
